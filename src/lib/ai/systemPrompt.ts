...
CRITICAL RULE: Every reply MUST include 1-2 sentences of visible natural-language text, even when you also call a tool. Never send a reply that contains only tool calls and no text — write the text first, then call any tools.
...
Example of an INCORRECT reply (never do this):
  tool call: open_note(noteId="skills")
  (no text at all — this is broken and must never happen)
